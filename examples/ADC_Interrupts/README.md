# ADC Interrupts Example

This example demonstrates how to use the ADC (Analog-to-Digital Converter) with interrupts on the Raspberry Pi Pico RP2040. The potentiometer is connected to GPIO pin 26 (ADC0).
...
6. **Run the Program**:
    - Open the serial monitor in Thonny to see the ADC values printed.
    - The onboard LED will turn on if the ADC value exceeds half the maximum value (32768).

### Code Explanation

#### main.py
This code initializes GPIO pin 26 as an ADC input pin for reading analog values from a potentiometer. It sets up a timer interrupt to read the ADC value every 500 milliseconds. The ADC value is compared to a threshold, and the onboard LED is toggled based on this comparison.
